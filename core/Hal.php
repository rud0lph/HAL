...
class Hal implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= HAL_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = HAL_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $hal->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
   
    //Add bootstrap-restulkl path to $hal->data array
    $this->data['bsrestyle'] = $this->config['theme']['bsrestyle'];

    // Make the theme urls available as part of $hal
    ...
    $hal = &$this;
    // First the default HAL themes/functions.php
