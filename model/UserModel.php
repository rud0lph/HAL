 ...
 * @package HalCore
 */
class UserModel extends Controller implements IHasSQL, ArrayAccess {
   ...
   * Constructor
   */
  public function __construct($hal=null) {
    parent::__construct($hal);
  ...
  public static function SQL($key=null) {
    $queries = array(
      ...
      'update password'         => "UPDATE User SET algorithm=?, salt=?, password=?, updated=TIMESTAMP(8) WHERE id=?;",
     );
  ...
  public function CreatePassword($plain, $algorithm=null) {
    $password = array(
      'algorithm'=>($algorithm ? $algoritm : Hal::Instance()->config['hashing_algorithm']),
      'salt'=>null
    );
