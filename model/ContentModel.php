 ...
 * @package HalCore
 */
class ContentModel extends Controller implements IHasSQL, ArrayAccess {
  ...
  public static function SQL($key=null, $args=null) {
    ...
    $queries = array(
      ...
      'insert content'          => 'INSERT INTO Content (content_key,type,title,data,filter,idUser,created) VALUES (?,?,?,?,?,?,?);',
      ...
      'update content'          => "UPDATE Content SET content_key=?, type=?, title=?, data=?, filter=?, updated=timestamp(8) WHERE id=?;",
     );
  ...
  public function Save() {
    ...
    if($this['id']) {
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array($this['key'], $this['type'], $this['title'], $this['data'], $this['filter'], $this->user['id'], null));
  ...
  public static function Filter($data, $filter) {
    switch($filter) {
      ...
      case 'htmlpurify': $data = nl2br(HTMLPurifierWrapper::Purify($data)); break;
